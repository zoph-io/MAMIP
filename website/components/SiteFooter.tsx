...
import { SIBLING_LINKS } from "@/lib/galaxy";
import { GITHUB_URL, MENU_GROUPS } from "@/lib/nav";

export default function SiteFooter() {
  return (
    <footer className="border-t border-zinc-200 dark:border-zinc-800 mt-16">
      {/* Sitemap. The topbar only carries three links, so this is what keeps
          every page one click away from every other page. */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 border-b border-zinc-200 dark:border-zinc-800">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          {MENU_GROUPS.map((group) => (
            <div key={group.id}>
              <p className="text-[10px] font-mono font-semibold uppercase tracking-widest text-zinc-400 dark:text-zinc-500 mb-3">
                {group.label}
              </p>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex flex-col items-center space-y-4">
          ...
          <p className="text-center text-xs text-zinc-400 dark:text-zinc-500 max-w-2xl">
            This is an unofficial archive and is not affiliated with, endorsed
            by, or sponsored by Amazon Web Services (AWS). AWS and related marks
